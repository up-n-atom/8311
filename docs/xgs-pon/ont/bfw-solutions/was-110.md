...
within Linux through an abstraction layer and kernel module `modinfo mod_sfp_i2c`.

There are several interfaces and utilities that provide read and/or write access to the EEPROM.
...
The EEPROM is write protected and can be unlocked by writing `12345678` to A2 (0x51) offset `0x7B`.
